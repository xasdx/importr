...
test("rejects ambiguous module names", t => {
  let error = t.throws(() => importr("module_"))
...
test("imports multiple modules", t => {
  let m = importr(["node_util", "module_a", "module_b"])
  t.true(m.node_util.isNull(null))
  t.is(m.module_a.data, "hello")
  t.is(m.module_b.behavior(), "hey")
})
...
test("fails when module list and alias list are different in length", t => {
  let error = t.throws(() => importr(["node_util", "module_a"], ["util"]))
